...
<link href="<?=$imglogos['image_path']?>" rel="icon">
  ...
  * License: https://bootstrapmade.com/license/
  ...
  <?php
      ...
      $querybg1 = "SELECT * FROM background WHERE bg_page='philosophy'";
      ...
      <div class="container">
          ...
          <div class="col-lg-12">
            <ul id="service-details-flters">
              ...
              <?php
                ...
                $query = "SELECT * FROM philosophy GROUP BY philosophy_tag;";
                $results = mysqli_query($db, $query);
                while($row = mysqli_fetch_assoc($results)){
                  if($row['philosophy_tag'] != 'None'){

               ?>

              <li class="text-capitalize" data-filter=".filter-<?= $row['philosophy_tag']; ?>"><?= $row['philosophy_tag']; ?></li>
        ...
        <form class="" action="index.php" method="post">
          ...
          <div class="row service-details-container" data-aos="fade-up" data-aos-easing="ease-in-out" data-aos-duration="500">
            ...
            $query = "SELECT * FROM philosophy WHERE philosophy_priority <> '0' ORDER BY philosophy_priority ASC;";
            $results = mysqli_query($db, $query);
            while($row = mysqli_fetch_assoc($results)){

              $img_id = $row['philosophy_intro_image'];
              $query = "SELECT * FROM images WHERE image_id='$img_id'";
              $r = mysqli_query($db, $query);
              $img = mysqli_fetch_assoc($r);
              if($row['philosophy_tag'] == 'None'){
                $service_tag = 'philosophy';
                echo '<div class="col-lg-4 col-md-6">';
              }
              else{
                $service_tag = $row['philosophy_tag'];
            ...
            ?>
              ...
              <div class="card">
                ...
                <div class="card-body">
                  <h5 class="card-title text-uppercase"><a href="philosophy.php?philosophy=<?= $row['philosophy_id']; ?>"><?= $row['philosophy_name']; ?></a></h5>
                  <p class="card-text"><?= $row['philosophy_shortintro']; ?></p>
                  <div class="read-more"><a href="philosophy.php?philosophy=<?= $row['philosophy_id']; ?>"><i class="icofont-arrow-right"></i> Read More</a></div>
                </div>
              </div>
            </div>
