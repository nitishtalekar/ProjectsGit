...
$display_alert = "";
function backup_tables($tables,$db,$name)
{
  ...
  $return  = '';
    ...
    $colname_query = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'feedback_temp'";
	...
	$handle = fopen('Backup/feedback_temp_'.$name.'-'.$cnt.'.sql','w+');
	fwrite($handle,$return);
	fclose($handle);
}
if(isset($_POST['backup'])){
  $name = mysqli_real_escape_string($db, $_POST['name']);
  backup_tables('feedback_temp',$db,$name);
